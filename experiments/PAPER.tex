...
The dynamic model successfully learned a balanced routing strategy (using $\sim$54-58% GDN in routed layers) and achieved comparable performance to the static baseline.
...
\begin{table}[h]
    ...
    \begin{tabular}{lccc}
        ...
        \textbf{Dynamic (Balanced)} & \textbf{4.43} & \textbf{34.65\%} & \textbf{Mixed ($\sim$54\% GDN L1, 58\% GDN L2)} \\
        \bottomrule
    \end{tabular}
    \caption{Performance Comparison after 1500 training steps with $\alpha=0.5$ load balancing}
...
\begin{itemize}
    \item \textbf{Layer 1:} 54.0\% GDN / 46.0\% Softmax (prefers slightly more linear attention)
    \item \textbf{Layer 2:} 57.7\% GDN / 42.3\% Softmax (stronger preference for linear attention)
\end{itemize}
...
Qualitative analysis of the routing decisions reveals interesting patterns. For example, in the sentence ``The quick brown fox...'', the model routes function words like ``The'', ``over'', and ``of'' to Softmax attention in Layer 1, while content words like ``quick'', ``brown'', and ``fox'' are routed to Linear attention. This suggests the model uses the more expensive Softmax attention for structural tokens that may require precise attending to previous context, while efficient Linear attention suffices for content tokens.

\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{images/token_routing_heatmap.png}
    \caption{Token-Level Routing Decisions. Blue indicates Linear Attention (GDN), Red indicates Softmax Attention. The model learns to route specific tokens to different mechanisms.}
    \label{fig:routing_heatmap}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{images/routing_confidence.png}
    \caption{Routing Confidence. The router makes confident decisions (probabilities close to 0 or 1) rather than uncertain ones.}
